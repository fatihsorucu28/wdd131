document.addEventListener("DOMContentLoaded", function() {
    ...
    // Initial call to generate all temple cards
    ...
    const largeFilter = document.getElementById('largeFilter');
    const smallFilter = document.getElementById('smallFilter');

    // Event listeners for filtering
    oldFilter.addEventListener('click', function(event) {
        event.preventDefault();
        const oldTemples = temples.filter(temple => parseInt(temple.dedicated.split(',')[0]) < 1900);
        generateTempleCards(oldTemples);
    });

    newFilter.addEventListener('click', function(event) {
        event.preventDefault();
        const newTemples = temples.filter(temple => parseInt(temple.dedicated.split(',')[0]) > 2000);
        generateTempleCards(newTemples);
    });

    largeFilter.addEventListener('click', function(event) {
        event.preventDefault();
        const largeTemples = temples.filter(temple => temple.area > 90000);
        generateTempleCards(largeTemples);
    });

    smallFilter.addEventListener('click', function(event) {
        event.preventDefault();
